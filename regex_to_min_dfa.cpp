...
void display_nfa() {
  cout << endl << endl;
  cout << "Phase 1: regex to nfa conversion using thompson's construction "
          "algorithm\n";
  cout << "--------------------------------------------------------------------"
          "----\n";
...
pair<int, vector<tuple<int, int, bool> > > minimize_dfa(vector<dst> dfa) {
    ...
    for (int i = 0; i < part.size(); i++) {
      for (int j = 0; j < 2; j++) {
        vector<pair<int, int> > trans(
            part[i].size());  /// Transitions for the states of partitions
        ...
        if (trans[0].first != trans[trans.size() - 1].first) {
          ...
          for (k = 1;
               k < trans.size() and (trans[k].first == trans[k - 1].first);
               k++) {
  ...
  for (int i = 0; i < (int)part.size(); i++) {
    ...
    get<0>(ret[i]) =
        (dfa[part[i][0]].a[0] >= 0) ? grp[dfa[part[i][0]].a[0]] : -1;
    get<1>(ret[i]) =
        (dfa[part[i][0]].a[1] >= 0) ? grp[dfa[part[i][0]].a[1]] : -1;
    ...
    for (unsigned i = 0; i < input.size(); i++) {
      if (curr_state >= 0) {
        if (input[i] == 'a')
          ...
          next_state = get<1>(min_dfa[curr_state]);
        if (next_state >= 0) {
          cout << input[i] << " : State " << curr_state << " -> State "
               << next_state << endl;
        } else
          cout << input[i] << " : State " << curr_state << " -> Trap State"
               << endl;
      } else
